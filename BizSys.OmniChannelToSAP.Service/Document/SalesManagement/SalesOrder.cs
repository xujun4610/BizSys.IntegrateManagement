...
namespace BizSys.OmniChannelToSAP.Service.Document.SalesManagement
{
    public class SalesOrder
    {
        ...
        public static Result CreateSalesOrder(ResultObjects order)
        {
            ...
            myDocuments.UserFields.Fields.Item("U_U_BillType").Value = order.BillType.ToUpper() == "COMMON" ? "11" : "12"; //11，12 仅限可选值 开票类型
            myDocuments.UserFields.Fields.Item("U_U_Voucher").Value = order.ServiceNumberManagementMoney; //代金券金额 ，double类型 注意
            if (!string.IsNullOrWhiteSpace(order.PrmtsContent))
                myDocuments.UserFields.Fields.Item("U_U_PrmtsContent").Value = order.PrmtsContent; //促销活动内容
            myDocuments.UserFields.Fields.Item("U_U_OCMDocEntry").Value = order.DocEntry.ToString(); //同步过来的OCM订单编号
            ...
            foreach (var item in order.SalesOrderItems)
            {
                ...
                myDocuments.Lines.UserFields.Fields.Item("U_U_Rebate").Value = item.Reference2; //返利情况
        ...
        /// <param name="order"></param>
        /// <returns></returns>
        public async static Task<Result> CreateSalesOrderAsync(ResultObjects order)
        {
            Result result = new Result();
            /*
             * 梅菲特奇葩要求
