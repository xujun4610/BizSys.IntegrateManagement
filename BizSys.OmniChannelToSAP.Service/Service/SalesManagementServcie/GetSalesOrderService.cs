...
namespace BizSys.OmniChannelToSAP.Service.Service.SalesManagementServcie
{
    public class GetSalesOrderService
    {
        ...
        {
            int resultCount = DataConvert.ConvertToIntEx(ConfigurationManager.AppSettings["ResultCount"], 5);
        ...
        public async static void GetSalesOrder4Test(int LastResultCount = -1, string LastDocEntry = null)
        {
            int resultCount = DataConvert.ConvertToIntEx(ConfigurationManager.AppSettings["ResultCount"], 5);
            string guid = "SalesOrder-" + Guid.NewGuid();
            string resultJson = string.Empty;
            ...
            {
                //下一页
                GetSalesOrder4Test(salesOrder.ResultObjects.Count, salesOrder.ResultObjects[salesOrder.ResultObjects.Count - 1].DocEntry.ToString());
            }
