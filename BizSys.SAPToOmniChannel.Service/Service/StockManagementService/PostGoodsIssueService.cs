...
{
    public class PostGoodsIssueService
    {
        public async static void PostGoodsIssue()
        {
            int resultCount = DataConvert.ConvertToIntEx(ConfigurationManager.AppSettings["GetGoodsIssue"], 30);
            ...
            {
                Logger.Writer("库存发货查询出错，错误信息：" + ex.Message);
                return;
            }
            ...
            #region 订单处理
            //反序列化
            GoodsIssueRootObject GoodsReceipt = JsonConvert.DeserializeObject<GoodsIssueRootObject>(resultJson);
            if (GoodsReceipt.ResultObjects.Count == 0) return;

            Logger.Writer(guid, QueueStatus.Open, "[" + GoodsReceipt.ResultObjects.Count + "]条库存发货开始处理。");
            Logger.Writer(guid, QueueStatus.Close, "库存发货信息：\r\n" + resultJson);
            //生成销售订单
            int mSuccessCount = 0;
            foreach (var item in GoodsReceipt.ResultObjects)
            {
                ...
                {
                    Logger.Writer("库存发货保存出错，错误信息：" + ex.Message);
                    return;
                }
